...
import { Bell, Search, Loader2, CheckCircle2, Play, Pause, X, Settings } from "lucide-react";
...
export function TopBar() {
  ...
  useEffect(() => {
    if (totalNotifications > prevNotifCount.current) setUnreadNotifications(true);
    prevNotifCount.current = totalNotifications;
  }, [totalNotifications]);

  useEffect(() => {
    if (!isLeadsPage) setSearchQuery("");
  ...
  useEffect(() => {
    const checkReminders = async () => {
      ...
      reminders.filter(r => !r.completed).forEach(r => {
        ...
        if (diffMins === 5 && !notified5m.current.has(r.id)) {
          ...
          if (notificationPreference === "both" || notificationPreference === "toast") toast.info(title, { description: body, duration: 10000 });
          if ((notificationPreference === "both" || notificationPreference === "os") && canUseOSNotifs) new Notification(title, { body, icon: "/icon.png" });
        ...
        if (diffMins === 1 && !notified1m.current.has(r.id)) {
          ...
          if (notificationPreference === "both" || notificationPreference === "toast") toast.warning(title, { description: body, duration: 15000 });
          if ((notificationPreference === "both" || notificationPreference === "os") && canUseOSNotifs) new Notification(title, { body, icon: "/icon.png" });
      ...
      // Pacing check (once every 30 min)
        ...
        if (currentMins >= startMins && currentMins <= endMins) {
          try {
            ...
            if (userId) {
              ...
              const { count } = await supabase.from("call_logs").select("*", { count: "exact", head: true }).eq("user_id", userId).gte("created_at", startOfDay);
              if (count !== null) {
                ...
                if (count < expectedCalls - 10) {
                  ...
                  if (notificationPreference === "both" || notificationPreference === "toast") toast.success(title, { description: body, duration: 8000 });
                  if ((notificationPreference === "both" || notificationPreference === "os") && canUseOSNotifs) new Notification(title, { body, icon: "/icon.png" });
                }
              }
            }
    ...
    return () => clearInterval(interval);
  }, [reminders, notified5m, notified1m, notifiedPowerShots, lastPaceCheck, notificationPreference]);

  return (
    <header className="h-16 bg-[var(--bolt-bg-depth-2)]/90 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-4 md:px-6 border-b border-[var(--bolt-border-color)] shadow-sm gap-2 md:gap-4 shrink-0">

      {/* Left Area: Power Shot Indicator or Spacer */}
      <div className="flex-1 flex items-center justify-start">
        {activePowerShot && (
          <div
            className="inline-flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-500 text-[10px] md:text-xs font-bold animate-in fade-in shrink-0"
          ...
          >
            ...
            <span className="hidden sm:inline">{activePowerShot} Calling</span>
            <span className="sm:hidden text-[10px] uppercase tracking-wider">{activePowerShot.replace("Power Shot ", "PS ")}</span>
          </div>
        )}
      </div>

      {/* Center Search — only on /leads or /applications */}
      {isLeadsPage ? (
      ...
      <div className="flex-1 flex items-center justify-end gap-2">
        ...
        <div className="relative" ref={notifRef}>
          <div
            className="cursor-pointer w-9 h-9 flex items-center justify-center rounded-lg hover:bg-[var(--bolt-hover-overlay)] transition-colors text-[var(--bolt-text-secondary)] hover:text-[var(--bolt-text-primary)]"
            onClick={() => {
          ...
          {showNotifications && (
            <GlassCard depth={2} className="absolute top-full mt-2 right-0 w-80 max-h-[400px] overflow-y-auto z-50 flex flex-col p-0 border border-[var(--bolt-border-color)] shadow-2xl rounded-xl">
              ...
              <div className="flex flex-col">
                {jobs.length > 0 && (
                  <div className="flex flex-col">
                    <div className="px-3 py-2 text-xs font-semibold text-[var(--bolt-text-tertiary)] uppercase tracking-wider bg-[var(--bolt-bg-depth-3)]">
                      Background Jobs
                    </div>
                    {jobs.map(job => (
                      <div key={job.id} className="p-3 border-b border-[var(--bolt-border-color)]/50 hover:bg-[var(--bolt-hover-overlay)] transition-colors group">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-medium text-[var(--bolt-text-primary)] line-clamp-1">{job.title}</span>
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            {job.status === "running" ? (
                              <button onClick={() => pauseJob(job.id)} className="p-1 hover:bg-[var(--bolt-hover-overlay-md)] rounded text-[var(--bolt-text-secondary)] hover:text-[var(--bolt-text-primary)]" title="Pause">
                                <Pause size={14} />
                              </button>
                            ) : job.status === "paused" ? (
                              <button onClick={() => startJob(job.id)} className="p-1 hover:bg-[var(--bolt-hover-overlay-md)] rounded text-[var(--bolt-text-secondary)] hover:text-[var(--bolt-text-primary)]" title="Resume">
                                <Play size={14} />
                              </button>
                            ) : null}
                            <button onClick={() => removeJob(job.id)} className="p-1 hover:bg-[var(--bolt-hover-overlay-md)] rounded text-[var(--bolt-text-secondary)] hover:text-rose-400" title="Dismiss">
                              <X size={14} />
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="w-full bg-[var(--bolt-bg-depth-4)] rounded-full h-1.5 overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-500 ${job.status === "completed" ? "bg-green-500" : job.status === "failed" ? "bg-red-500" : "bg-[var(--bolt-accent)]"}`}
                          ...
                          return (
                            <div key={r.id} className="p-3 hover:bg-[var(--bolt-bg-depth-3)] transition-colors">
                              <div className="flex items-start justify-between gap-3">
                                ...
                                <div className={`text-[10px] font-medium whitespace-nowrap px-1.5 py-0.5 rounded-sm ${isToday ? "bg-[var(--bolt-accent)]/10 text-[var(--bolt-accent)]" : "bg-[var(--bolt-bg-depth-3)] text-[var(--bolt-text-tertiary)]"}`}>
        ...
        <Link
          href="/settings"
          className="md:hidden w-8 h-8 rounded-full bg-[var(--bolt-bg-depth-3)] hover:bg-[var(--bolt-bg-depth-4)] flex items-center justify-center text-[var(--bolt-text-secondary)] transition-colors"
          title="Settings"
        >
          <Settings size={16} />
        </Link>
        ...
        <Link
          href="/profile"
          title="Profile"
          className="w-8 h-8 rounded-full bg-gradient-to-tr from-[var(--bolt-accent)] to-[#fde047] shadow-sm ml-2 border border-black/10 flex items-center justify-center shrink-0 hover:opacity-90 transition-opacity"
        >
